...
class UNet2(nn.Module):
    ...
    def __init__(self, image_channels: int = 3, n_channels: int = 64,
        ...
        * `n_blocks` is the number of `UpDownBlocks` at each resolution
        ...
        # Time embedding layer. Time embedding has `time_channels` channels
        time_channels = n_channels * 12
        self.time_emb = TimeEmbedding(time_channels)

        time_emb_channels = 2*time_channels
        ...
        for i in range(n_resolutions):
            ...
            # Add `n_blocks`
            for _ in range(n_blocks):
                down.append(DownBlock(in_channels, out_channels, time_emb_channels, is_attn[i]))
                in_channels = out_channels
            # Down sample at all resolutions except the last
            if i < n_resolutions - 1:
                down.append(Downsample(in_channels))
        ...
        # Middle block
        self.middle = MiddleBlock(out_channels, time_emb_channels, is_attn[-1]) # false for middle block not having attention
        ...
        for i in reversed(range(n_resolutions)):
            ...
            out_channels = in_channels
            for _ in range(n_blocks):
                up.append(UpBlock(in_channels, out_channels, time_emb_channels, is_attn[i]))
            # Final block to reduce the number of channels
            out_channels = in_channels // ch_mults[i]
            up.append(UpBlock(in_channels, out_channels, time_emb_channels, is_attn[i]))
            in_channels = out_channels
    ...
    def unet_forward(self, x: torch.Tensor, t_c: torch.Tensor):
        # Get image projection
        x = self.image_proj(x)
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, c: torch.Tensor):
        ...
        * `t` has shape `[batch_size]`
        ...
        # Concatenate time embeddings with condition
        t_c = torch.cat((t, c), dim=1)

        return self.unet_forward(x, t_c)
